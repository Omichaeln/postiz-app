...
import { createAgentRunActivities } from '@oremedia/activities';
...
/**
 * Spec 4.4: worker-core hosts task queue `agents` (agentRunWorkflowV1 and its signal relay). Workflow code is
...
export async function startAgentsWorker(
  ...
  const worker = await Worker.create({
    ...
    activities: createAgentRunActivities(runtime),
